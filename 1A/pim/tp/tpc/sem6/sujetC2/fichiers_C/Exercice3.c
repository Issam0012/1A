...
#define NB_CARTES (4*NB_VALEURS) // Attention : les parentheses sont importantes !
// Bonne pratique : Toujours parenthéser les expressions écrites dans constantes
// pré-processeur. On s'assure que cette expression est réalisée de façon prioritaire.
...
carte * piocher(jeu le_jeu, t_main* main){
    ...
    if (i == NB_CARTES) {
        ...
        // Reallouer la mémoire pour enregistrer une carte de plus dans la main.
                //t_main* main_temp;
        main = realloc(main, (main->nb + 1)*sizeof(carte));
        // Penser à l'echec de la reallocation
        if (main==NULL) {
            printf("réallocation échoué");
        }
        //main = main_temp;
        // Copier la carte_piochee dans la main
        main->main[main->nb] = *carte_piochee;
