...
procedure Evaluer_Alea_TH is
    ...
    is
        ...
        package TH_Integer is
                new TH (CAPACITE => Borne, T_Cle => Integer, T_Donnee => Integer, hachage => identite);
        use TH_Integer;
    ...
    procedure comparaison is
        new Pour_Chaque_th (comparaison);

    nombre, frequence:integer;
    liste:T_TH;

    begin --c'est la même procédure que lca, j'ai juste ajouté la fonction identite pour pouvoire instancier le package TH
        Max:=1;
        Initialiser(liste);
        for i in 1..Borne loop
            Enregistrer (liste, i, 0);
        end loop;

        for i in 1..Taille loop
            Get_Random_Number(nombre);
            frequence := La_Donnee (liste, nombre);
            Supprimer(liste, nombre);
            Enregistrer (liste, nombre, frequence + 1);
