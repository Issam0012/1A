...
public class Main {
	...
	private static void repondreQuestions(Reader in) {
		...
		int nombreVente = pdvs.size();


		// Afficher le nombre de services existants
		int nombreServices = 0;
		Iterator<Long> iterateur = pdvs.keySet().iterator();
		while (iterateur.hasNext()) {
			nombreServices = pdvs.get(iterateur.next()).getServices().size() + nombreServices;
		}
	 	iterateur.forEachRemaining(nombreServices = pdvs.get(iterateur.next()).getServices().size() + nombreServices);
		System.out.print("le nombre total des services est : " + nombreServices);


		// Afficher les services fournis
		Iterator<Long> iterateur2 = pdvs.keySet().iterator();
		while (iterateur2.hasNext()) {
			System.out.print("Les services de la station d'identifiant : " + iterateur2.next() + "sont : ");
			System.out.println(pdvs.get(iterateur2.next()).getServices());
		}
