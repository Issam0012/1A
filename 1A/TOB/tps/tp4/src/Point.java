...
//  Gestion de la couleur
	...
	/** Dessiner un point dans un écran.
	  * @param ecran l'écran qu'on veut dessiner sur
	  */
	public void dessiner(Ecran ecran) {
		ecran.dessinerPoint(this.getX(), this.getY(), this.couleur);
	}
	
	/** utiliser l'affichage svg pour afficher un point.
	  * @param affsvg l'afficherur qu'on va utiliser 
	  */
	public void dessiner(AfficheurSVG affsvg) {
		affsvg.dessinerPoint(this.getX(),this.getY(),this.getCouleur());
	}
	
	/** afficher un point en mode afficheurTexte
	  * @param afftxt l'afficherur qu'on va utiliser 
	  */
	public void dessiner(AfficheurTexte afftxt) {
		afftxt.dessinerPoint(this.getX(),this.getY(),this.getCouleur());
	}
