...
for k=1:length(EbNo)
    ...
    % Filtrage et échantillonnage des 3 signaux déphasés
    signal_recu_Phi1 = filter(hr, 1, enveloppe_bruite_dephase_Phi1);
    signal_echan_1 = signal_recu_Phi1(n0:Ns:end);

    signal_recu_Phi2 = filter(hr, 1, enveloppe_bruite_dephase_Phi2);
    signal_echan_2 = signal_recu_Phi2(n0:Ns:end);

    signal_recu_Phi3 = filter(hr, 1, enveloppe_bruite_dephase_Phi3);
    signal_echan_3 = signal_recu_Phi3(n0:Ns:end);

    % Estimation des angles phi
    Phi1_estime = 1/2 * angle(sum(signal_echan_1.^2));
    Phi2_estime = 1/2 * angle(sum(signal_echan_2.^2));
    Phi3_estime = 1/2 * angle(sum(signal_echan_3.^2)) + pi;

    % Correction des signaux reçus
    signal_echan_1 = signal_echan_1 * exp(-i*Phi1_estime);
    signal_echan_2 = signal_echan_2 * exp(-i*Phi2_estime);
    signal_echan_3 = signal_echan_3 * exp(-i*Phi3_estime);

    %  calcul des décisions
    decision = real(signal_echan_1);
    decision40 = real(signal_echan_2);
    decision100 = real(signal_echan_3);

    % Affectation des TEB
    TEB_Theo(k) = qfunc(real(exp(i*Phi*pi/180))*sqrt(2*10^(EbNo(k)/10)));
