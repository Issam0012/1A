...
for i = 1:m
   for j = 1:n
      ...
      f_plot(i,j) = 1/2*norm(res_plot)^2;
   end
end
...
%
    ...
    H_f=0;
    for i=1:7
        H_r = [0, donnees(i,1).*exp(-beta(2)*donnees(i,1)); donnees(i,1).*exp(-beta(2)*donnees(i,1)), -beta(1)*(donnees(i,1)^2).*exp(-beta(2)*donnees(i,1))];
        a=residu(beta);
        H_f=H_f + a(i)*H_r;
    end
    H_f=H_f+J_residu(beta)'*J_residu(beta);
end
