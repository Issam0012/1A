...
| Leaf _ -> 1
| Node(a,b) -> 1 + (num_nodes a) + (num_nodes b)
...
| Node(a,b) -> (num_values a) + (num_values b)
 ...
 *)
let rec bits n addr =let i = addr mod 2 in match i with
| 0 -> if n>0 then 0::(bits (n-1) (addr/2)) else []
| 1 -> if n>0 then 1::(bits (n-1) (addr/2)) else []
| _ -> failwith"impossible d'arriver ici puisqu'on travaille modulo 2"
 ...
 *)
let rec search tr addrl = match addrl with
| [] -> (match tr with | Leaf c -> c | _ -> failwith"")
| t::q -> match tr with
            | Node(a,b) -> if t=0 then search a q else search b q
            | Leaf c -> c
 ...
 *   ailleurs
 *)
let rec sprout x addrl = match addrl with
| [] -> Leaf x
| [e] -> if e = 0 then Node(Leaf x, Leaf _0) else Node(Leaf _0, Leaf x)
| t::q -> if t = 0 then Node((sprout x q), Leaf _0) else Node(Leaf _0, (sprout x q))
 ...
 *   devrait pas arriver !
 *)
let rec update tr x addrl = match tr with
| Leaf _ -> (match addrl with
            | [] -> sprout x addrl
            | t::q -> if t=0 then Node(sprout x q,tr) else Node(tr,sprout x q))
    
| Node(a,b) -> match addrl with
                | [] -> failwith"on sera jamais ici"
                | t::q -> if t=0 then Node(update a x q,b) else Node(a,update b x q)
