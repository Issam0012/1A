...
struct
    ...
    let clear bs = (bs,Leaf _0)

    (* Implémentation de read. *)
    let read (bs, tr) addr = let value = (search tr (bits (pow2 bs) addr)) in if (addr> pow2 bs) then raise OutOfBound else value

    (* Implémentation de write. *)
    let write (bs, tr) addr value = let liste = update tr value (bits (pow2 bs) addr) in if addr > (pow2 bs) then raise OutOfBound else (bs, liste)
end
