...
evalue un polynome en un point donné avec la convention 0 si la liste polynome est vide
poly : un polynome codé avec une float list
nbr : le nombre voulu (float)
Retour : le polynome évalué à ce point
*)

let evalue polynome nbr = List.fold_right (fun a s -> s*.nbr +. a) polynome 0.

let%test _ = evalue [] 0. = 0.
let%test _ = evalue [1.;2.;3.] 2. = 17.
let%test _ = evalue [1.;2.;1.] 2. = 9.
let%test _ = evalue [1.] 10. = 10.
...
let rec retract l = match l with
| [] -> []
| t::q -> if t=0. && q=[] then [] else t::(retract q)
...
let scal_mult scalaire polynome = List.map (fun t -> scalaire *. t) polynome
...
let plus polynome1 polynome2 = List.map2 (fun a b -> a +. b) polynome1 polynome2
...
let get (Noeud(a,b,c)) n = match n with | 1 -> a | 2-> b | 3 ->c | _->failwith"entre 1 et 3 forcément"
let decoder code (Noeud(arb1,arb2,arb3)) = let l = decompose_int code in
let rec decoder_aux liste trio trioOrig=
  match liste with
  | [] -> []
  | t::q -> 
    let i = get trio t in match i with
              | Vide -> raise CodeNonValide
              | Lettre a -> [a] @ (decoder_aux q trioOrig trioOrig)
              | Noeud(arbre1,arbre2,arbre3) -> (decoder_aux q (arbre1) trioOrig) @ (decoder_aux q (arbre2) trioOrig) @ (decoder_aux q (arbre3) trioOrig)
              
in recompose_chaine (decoder_aux l (Noeud(arb1,arb2,arb3)) (Noeud(arb1,arb2,arb3)))
...
*)
let arbre_to_liste arbre_encodage =
  let rec arbre_to_liste_aux arbre acc =
    match arbre with
    | Vide -> []
    | Lettre c -> [(c,acc)]
    | Noeud(arb1,arb2,arb3) -> (arbre_to_liste_aux arb1 (acc*10+1)) @ (arbre_to_liste_aux arb2 (acc*10+2)) @ (arbre_to_liste_aux arb3 (acc*10+3))
  in arbre_to_liste_aux arbre_encodage 0
...
let rec taille n = if n = 0 then 0. else 1. +. (taille (n/10))
let encoder mot arbre = let ls = decompose_chaine mot in
  let rec encoder_aux l arb =
    match l with
      | [] -> 0
      | t::q -> let code = List.assoc_opt t (arbre_to_liste arbre) in if code = None then raise MotNonValide else let intcode = Option.get code in
              let qcode = (encoder_aux q arbre) in
              qcode + (int_of_float (10.**(taille qcode))) * intcode
  in encoder_aux ls arbre
