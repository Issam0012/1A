...
struct
  ...
  let get_branche c (Noeud liste) = List.assoc_opt c liste
  ...
  let dechiffrer l arbre =
  let rec dechiffrer_aux ls arbre_chiffrement arbreOrig = 
    match (ls,arbre_chiffrement) with
      | ([], Noeud(_)) -> raise CodeNonValide
      | ([], Lettre a) -> [a]
      | (_::_, Lettre a) -> [a] @ dechiffrer_aux ls arbreOrig arbreOrig
      | (t::q, _) -> let branche = get_branche t arbre_chiffrement in 
      if branche = None then raise CodeNonValide else dechiffrer_aux q (Option.get branche) arbreOrig
  in dechiffrer_aux l arbre arbre
  ...
  let rec arbre_to_liste (Noeud (l)) = 
   match l with
  | [] -> []
   | (t,Noeud(q))::lq -> let l1 = arbre_to_liste (Noeud(q)) in (List.map (fun (a,b)-> (a,[t]@b)) l1) @ arbre_to_liste (Noeud(lq))
   | (t,Lettre c)::q -> [c, [t]] @ arbre_to_liste (Noeud(q))
 ...
 let rec element c liste = if liste=[] then raise DonneeNonValide else let (t,l)::q = liste in if t=c then l else element c q
  let rec chiffrer liste arbre  =
  let l = arbre_to_liste arbre in 
    match liste with
    | [] -> []
    | t::lq -> (element t l) @ (chiffrer lq arbre)
