...
(******************************************************************************)
(*   fonction de suppression d'un élément d'une arbre                         *)
(*   signature  : retrait_arbre : 'a list -> 'a arbre -> 'arbre'              *)
(*   paramètres : - une liste d'éléments (caractères dans le cas d'un dico)   *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre dont l'élément est retiré                           *)
(******************************************************************************)
let rec retrait_arbre lc (Noeud(b,tlb::qlb)) = 
   match lc with
  | [] -> if b then (if not (qlb=[]) then Noeud(b,qlb) else Noeud(false, qlb)) else failwith("Element inexsitant")
  | c::qlc ->
     match recherche c (tlb::qlb) with
     | None -> failwith("Element inexsitant")
     | Some a -> retrait_arbre qlc a
