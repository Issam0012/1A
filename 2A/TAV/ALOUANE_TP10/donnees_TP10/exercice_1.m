...
% Comparaison de signal restitué au signal original
if sum(size(signal_restitue)==size(y))==length(size(y))
    if all(abs(signal_restitue-y) <= 1e-10)
	    disp('Les signaux sont identiques')
    else
        disp('C pas bon mec !')
    end
else
