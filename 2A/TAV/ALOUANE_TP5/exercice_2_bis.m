...
% Zone de doute
D = zeros(size(u_0,1),size(u_0,2));
for i=1:size(u_0,1)
    for j=1:size(u_0,2)
        if (u_0(i,j,1)==255 & u_0(i,j,2)==255 & u_0(i,j,3)==0)
            D(i,j) = 255;
        end
    end
end
...
f_carac = (D==0);
W = spdiags(f_carac(:),0,nb_pixels,nb_pixels);
b = W*u_0;
...
while convergence > 1e-3
	...
	% Iteration (6) :
	u_kp1 = inpainting(b,u_k,lambda,Dx,Dy,epsilon,W);	
