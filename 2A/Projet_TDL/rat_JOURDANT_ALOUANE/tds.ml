...
let rec trouverGlobalement tds nom =
  match tds with
  | Nulle -> []
  | Courante (m,c) -> (find_all c nom)@(trouverGlobalement m nom)

let rec trouver_loop liste = 
    match liste with
    | [] -> false
    | t::q -> match info_ast_to_info t with
              | InfoLoop (_) -> true
              | _ -> false || (trouver_loop q)

let%test _ = 
let tds = creerTDSMere() in
let ix = info_to_info_ast (InfoVar ("x", Rat, 0, "SB")) in
let iy = info_to_info_ast (InfoVar ("y", Bool, 3, "LB")) in
ajouter tds "x" ix;
ajouter tds "y" iy;
let tdsf = creerTDSFille(tds) in
let ix2 = info_to_info_ast (InfoLoop ("x")) in
let iz = info_to_info_ast (InfoVar ("z", Rat, 4, "LB")) in
ajouter tdsf "x" ix2;
ajouter tdsf "z" iz;
trouver_loop (trouverGlobalement tdsf "x")