...
let rec analyse_placement_instruction i depl reg =
  ...
  | AstType.ConditionnelleOptionnel (c,b) -> 
      let bp = (analyse_placement_bloc b depl reg) in
      (AstPlacement.ConditionnelleOptionnel (c, bp), 0)
  | AstType.Empty -> (AstPlacement.Empty, 0)
  (* Pas de traitement particulier pour cette étape*)
  | AstType.Loop (b) -> 
    let nb = analyse_placement_bloc b depl reg in
  ...
  | AstType.LoopNom (n,b) -> 
    let nb = analyse_placement_bloc b depl reg in
    (AstPlacement.LoopNom(n,nb), 0)
  | AstType.BreakNom(n) -> (AstPlacement.BreakNom(n), 0)
  | AstType.ContinueNom(n) -> (AstPlacement.ContinueNom(n), 0)
  (* | AstType.AffectationPointeur(a, e) ->  (AstPlacement.AffectationPointeur(a, e), 0)
     *)
