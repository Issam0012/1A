...
function Lagrangien_Augmente(algo,fonc::Function,contrainte::Function,gradfonc::Function,
    ...
    while continu 
        xk = xk_1
        L(x) = fonc(x) + lambdak' * contrainte(x) + (muk/2)*norm(contrainte(x))^2
        gradL(x) = gradfonc(x) + lambdak' * grad_contrainte(x) + muk * grad_contrainte(x)*contrainte(x)
        hessL(x) = hessfonc(x) + lambdak' * hess_contrainte(x) + muk * (hess_contrainte(x)*contrainte(x) + grad_contrainte(x)*grad_contrainte(x)')
        if algo=="newton"
            xk_1,~,~,~ = Algorithme_De_Newton(L,gradL,hessL,xk,[])
        else
            xk_1,~,~,~ = Regions_De_Confiance(algo,L,gradL,hessL,xk,[])
        end
        if norm(contrainte(xk_1))<=etak
            lambdak = lambdak + muk*contrainte(xk_1)
            muk = muk
            epsilon = epsilon/muk
            etak = etak/(muk^beta)
        else
        ...
        append!(lambdaks, [lambdak])
        append!(muks, [muk])
        
        iter = iter + 1
        
        continu = norm(gradL(xk_1))>epsilon && (iter<itermax)
        if norm(gradL(xk_1))<=epsilon
            flag = 0
        elseif iter==itermax
            flag = 1
        end
