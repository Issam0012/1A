...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    if options == []
        ...
        while continu
            ...
            f1 = f(x)
            nb_iters = nb_iters + 1
            continu = (nb_iters<max_iter) && (norm(fmin-f1)>epsilon*max(Tol_rel*abs(f1),Tol_abs)) && (norm(xmin-x)>epsilon*max(Tol_rel*norm(x),Tol_abs)) && (norm(gradf(xmin))>max(Tol_rel*norm(grad0),Tol_abs))
            if (norm(gradf(xmin))<=max(Tol_rel*norm(grad0),Tol_abs))
                flag = 0
            elseif (norm(xmin-x)<=epsilon*max(Tol_rel*norm(x),Tol_abs))
                flag = 1
            elseif (norm(fmin-f1)<=epsilon*max(Tol_rel*abs(f1),Tol_abs))
                flag = 2
