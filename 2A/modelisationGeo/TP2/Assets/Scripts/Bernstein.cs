...
public class Bernstein : MonoBehaviour
{
    ...
    void buildPolysBernstein() {
        ...
        {
            List<Vector2> l_val_poly = new List<Vector2>();
            for (float t = 0.0f;t<=1;t=t+pas)
            {
            	float y = KparmiN(i,nombrePolynomesBernstein)* (float) Math.Pow(t, i)* (float) Math.Pow((1 - t), nombrePolynomesBernstein - i);
                l_val_poly.Add(new Vector2(t,y));
            }
            ListePoints.Add(l_val_poly);
        }
    }
